...
public class PlaylistService
{
    ...
    private async Task InvalidatePlaylistsCacheAsync(CancellationToken cancellationToken)
    {
            ...
            .Select(p => new PlaylistListItemDto
            {
                ...
                CollageCovers = p.Tracks
                    ...
                    .Select(pt => pt.Track!.IsExternal
                        ...
                        : (pt.Track.CoverImageRelativePath ?? pt.Track.Album!.CoverImageRelativePath))
                    .Where(url => url != null)
                    .Select(url => url!)
                    .ToList(),
            })
    ...
    /// <summary>Окончательно удаляет уже soft-deleted плейлист (из корзины) — не дожидаясь
    /// 30-дневной фоновой очистки. Треки плейлиста не трогаем — удаляются только связи PlaylistTracks.</summary>
        ...
        Guid playlistId, Guid currentUserId, string userRoles, CancellationToken cancellationToken = default)
    {
        ...
        await _context.Downloads
            .Where(d => d.Type == DownloadType.Playlist && d.ItemId == playlistId)
    ...
    public async Task<ServiceResult<IReadOnlyList<PlaylistListItemDto>>> GetAiPlaylistsAsync(string baseUrl, CancellationToken cancellationToken = default)
    {
        var result = await _context.Playlists
            .AsNoTracking()
            .Where(p => !p.IsDeleted && !p.IsPrivate)
            .Where(p => p.Title.Contains("ШІ Мікс") || p.Title.Contains("AI Mix") || p.Slug.Contains("ai-mix"))
        ...
        if (!result.Any())
        {
            result = await _context.Playlists
                .AsNoTracking()
                .Where(p => !p.IsDeleted && !p.IsPrivate)
                .OrderByDescending(p => p.CreatedAt)
                .Take(8)
        ...
        // Локальные (не-внешние) обложки хранятся в БД как относительный путь - без этого
        // фронтенд получит что-то вроде "covers/xyz.jpg" и попробует загрузить картинку
        // относительно своего собственного origin, а не gateway.
        foreach (var item in result)
        {
            item.CollageCovers = item.CollageCovers
    ...
    private static PlaylistDto MapToDto(Playlist p, string baseUrl = "", bool isLiked = false)
    {
        return new PlaylistDto(
            ...
            p.Tracks?.Select(pt => {
                ...
                var coverUrl = pt.Track?.IsExternal == true
                    ? pt.Track.ExternalCoverUrl
